...
"""
# NOTE: UI strings in this file were localized from Chinese to Japanese.
...
"""
        ...
        if model_kind == "custom_voice":
            with gr.Row():
                with gr.Column(scale=2):
                    text_in = gr.Textbox(
                        label="Text (合成するテキスト)",
                        lines=4,
                        placeholder="Enter text to synthesize (合成するテキストを入力).",
                    )
                    with gr.Row():
                        lang_in = gr.Dropdown(
                            label="Language (言語)",
                    ...
                    instruct_in = gr.Textbox(
                        label="Instruction (Optional) (制御指示・任意)",
                        lines=2,
                        placeholder="e.g. Say it in a very angry tone (例：とても悲しい口調で話して).",
                    )
                    btn = gr.Button("Generate (生成)", variant="primary")
                with gr.Column(scale=3):
                    audio_out = gr.Audio(label="Output Audio (合成結果)", type="numpy")
                    err = gr.Textbox(label="Status (ステータス)", lines=2)

            def run_instruct(text: str, lang_disp: str, spk_disp: str, instruct: str):
                try:
                    if not text or not text.strip():
                        return None, "Text is required (テキストは必須です)."
                    if not spk_disp:
                        return None, "Speaker is required (話者の選択が必要です)."
                    ...
                    return _wav_to_gradio_audio(wavs[0], sr), "Finished. (生成完了)"
            ...
            with gr.Row():
                with gr.Column(scale=2):
                    ...
                    design_in = gr.Textbox(
                        label="Voice Design Instruction (音色の説明)",
                        lines=3,
            ...
            def run_voice_design(text: str, lang_disp: str, design: str):
                try:
                    ...
                    if not design or not design.strip():
                        return None, "Voice design instruction is required (音色の説明は必須です)."
                    language = lang_map.get(lang_disp, "Auto")
                    kwargs = _gen_common_kwargs()
            ...
            with gr.Tabs():
                with gr.Tab("Clone & Generate (クローンして合成)"):
                    with gr.Row():
                        with gr.Column(scale=2):
                            ref_audio = gr.Audio(
                            ...
                            ref_text = gr.Textbox(
                                label="Reference Text (参照音声テキスト)",
                                lines=2,
                                placeholder="Required if not set use x-vector only (use x-vector only を未選択の場合は必須).",
                            )
                            xvec_only = gr.Checkbox(
                                label="Use x-vector only (話者ベクトルのみ使用・品質低下の可能性・参照音声テキスト不要)",
                                value=False,
                            )

                        with gr.Column(scale=2):
                            text_in = gr.Textbox(
                                label="Target Text (合成するテキスト)",
                                lines=4,
                                placeholder="Enter text to synthesize (合成するテキストを入力).",
                            )
                            lang_in = gr.Dropdown(
                    ...
                    def run_voice_clone(ref_aud, ref_txt: str, use_xvec: bool, text: str, lang_disp: str):
                        try:
                            if not text or not text.strip():
                                return None, "Target text is required (合成するテキストは必須です)."
                            at = _audio_to_tuple(ref_aud)
                            if at is None:
                                return None, "Reference audio is required (参照音声のアップロードが必要です)."
                            if (not use_xvec) and (not ref_txt or not ref_txt.strip()):
                                return None, (
                                    "Reference text is required when use x-vector only is NOT enabled.\n"
                                    "(use x-vector only 未選択時は参照音声テキストが必須。未入力の場合は use x-vector only を選択してください（品質が低下します）。)"
                                )
                            language = lang_map.get(lang_disp, "Auto")
                            kwargs = _gen_common_kwargs()
                ...
                with gr.Tab("Save / Load Voice (クローン音色の保存/読み込み)"):
                    with gr.Row():
                        with gr.Column(scale=2):
                            gr.Markdown(
                                """
...
(参照音声と参照テキストをアップロードし、use x-vector only モードの有無を選んで再利用可能な音色ファイルとして保存)
"""
                            )
                            ref_audio_s = gr.Audio(label="Reference Audio (参照音声)", type="numpy")
                            ...
                            save_btn = gr.Button("Save Voice File (音色ファイルを保存)", variant="primary")
                            prompt_file_out = gr.File(label="Voice File (音色ファイル)")

                        with gr.Column(scale=2):
                            gr.Markdown(
                                """
### Load Voice & Generate (音色を読み込み合成)
Upload a previously saved voice file, then synthesize new text.  
(保存済みのプロンプトファイルをアップロードし、新しいテキストを入力して合成)
"""
                            )
                            prompt_file_in = gr.File(label="Upload Prompt File (プロンプトファイルをアップロード)")
                            text_in2 = gr.Textbox(
                                label="Target Text (合成するテキスト)",
                                lines=4,
                                placeholder="Enter text to synthesize (合成するテキストを入力).",
                        ...
                        with gr.Column(scale=3):
                            audio_out2 = gr.Audio(label="Output Audio (合成結果)", type="numpy")
                            err2 = gr.Textbox(label="Status (ステータス)", lines=2)
                    ...
                    def load_prompt_and_gen(file_obj, text: str, lang_disp: str):
                        try:
                            if file_obj is None:
                                return None, "Voice file is required (音色ファイルのアップロードが必要です)."
                            if not text or not text.strip():
                                return None, "Target text is required (合成するテキストは必須です)."
                            ...
                            if not isinstance(payload, dict) or "items" not in payload:
                                return None, "Invalid file format (ファイル形式が正しくありません)."

                            items_raw = payload["items"]
                            if not isinstance(items_raw, list) or len(items_raw) == 0:
                                return None, "Empty voice items (音色が空です)."

                            items: List[VoiceClonePromptItem] = []
                            for d in items_raw:
                                if not isinstance(d, dict):
                                    return None, "Invalid item format in file (ファイル内部形式が不正です)."
                                ref_code = d.get("ref_code", None)
                                if ref_code is not None and not torch.is_tensor(ref_code):
                                    ref_code = torch.tensor(ref_code)
                                ref_spk = d.get("ref_spk_embedding", None)
                                if ref_spk is None:
                                    return None, "Missing ref_spk_embedding (話者ベクトルが不足しています)."
                            ...
                            return None, (
                                ...
                                f"(音色ファイルの読み込み/使用に失敗しました。形式や内容を確認してください)\n"
                                f"{type(e).__name__}: {e}"
                            )
...
- 音声は AI モデルが自動生成/合成したもので、体験・デモ目的のみです。不正確または不適切な内容が含まれる可能性があり、開発者/運営者の見解や専門的助言を示すものではありません。ユーザーは使用・共有・依拠に伴うすべてのリスクと責任を負うものとします。適用法令で許容される最大限の範囲で、開発者/運営者は本音声の使用または使用不能により生じた直接・間接・付随的・結果的損害について責任を負いません（法令により別途定めがある場合を除く）。本サービスを用いて違法・有害・誹謗中傷・詐欺・ディープフェイク・プライバシー/肖像/著作権/商標侵害等のコンテンツを意図的に生成・複製することを禁止します。ユーザーがプロンプトや素材等により違法または侵害行為を行った/助長した場合、その法的責任はユーザーが負い、開発者/運営者は責任を負いません。
"""
        )
